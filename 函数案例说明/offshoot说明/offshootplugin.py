...
class Plugin:
    ...
    @classmethod
    def on_install(cls):
        print("\n\n%s was installed successfully 安装成功!" % cls.__name__)
    ...
    def install(cls):
        ...
        manifest = offshootManifest.Manifest()
    ...
    def verify_plugin_dependencies(cls):
        print("\nOFFSHOOT PLUGIN INSTALL: 验证插件依赖项是否已经安装…...\n")
        ...
        print("missing_plugin_names : ", len(missing_plugin_names),missing_plugin_names)

        if len(missing_plugin_names):
            raise PluginError("不满足一个或多个插件依赖项:%s。请在继续之前安装它们..." % ", ".join(missing_plugin_names))
    ...
    def install_files(cls):
        ...
        print("pluggable_classes : ",pluggable_classes, cls.files)
    ...
    def install_configuration(cls):
        ...
        print(len(offshoot.config["file_paths"]["config"].split(os.sep)))
        print(offshoot.config["file_paths"]["config"].split(os.sep))
        print(cls.config,not len(cls.config or dict()))
